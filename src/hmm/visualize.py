...
from src.utils.paths import processed_dir as _processed_dir, raw_dir
...
def _wrap_feature_list(features: list[str], per_line: int = 5) -> str:
    ...
    chunks = [
        ", ".join(features[i : i + per_line]) for i in range(0, len(features), per_line)
    ]
    return "\n  ".join(chunks)
...
def _draw_two_week_panel(
    ...
    if (
        today_actual is not None
        and today_actual_ts is not None
        and len(today_actual) > 0
    ):
    ...
    ax.plot(
        ...
        linestyle="-",
        zorder=3,
        label=f"{in_data_label} in-data  RMSE=${in_data_rmse:.2f}{_ar2_str}",
    )
    ...
    if plus_in_pred is not None and plus_in_ts is not None:
        ...
        ax.plot(
            ts_for_plus,
            plus_in_pred,
            color=_XGB_PLUS_COLOR,
            linewidth=1.0,
            alpha=_INDATA_ALPHA,
            linestyle="-",
            zorder=3,
            label=f"{in_data_label}+ in-data  RMSE=${plus_rmse:.2f}{_plus_ar2_str}",
        )
    ...
    if current_regime is not None:
        feat_line = (
            ("Features:\n  " + _wrap_feature_list(feature_names))
            if feature_names
            else ""
        )
...
def run(config: dict) -> None:
    ...
    ax1.axvline(
        sol_close.index[-1], color="#555555", linewidth=0.8, linestyle=":", zorder=3
    )
    ax1.plot(
        future_ts_hmm,
        exp_price_hmm,
    ...
    ax2.set_title(
        ...
        + (
            ...
            if xgb_results.get("xgb_plus_exp") is not None
            else ""
        ),
        color="white",
        fontsize=10,
    ...
    ax3.set_title(
        ...
        if not np.isnan(np_r["np_exp"][-1])
        else "NeuralProphet direct forecast",
        color="white",
        fontsize=10,
        loc="left",
        pad=8,
